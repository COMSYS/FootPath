...
/**
 * This class extends LinkedList<Location> to provide a means to collect GPS
 * Locations, return the total distance between point 1 to point n, and helper
 * functions to draw a path and export the data.
 * 
 * @author Paul Smith
 * 
 */
public class LocationHistory extends LinkedList<Location> {
	...
	private int[] convertToPixelLocation(Location gpsLocation, Location center,
		...
		if (gpsLocation != null && center != null) {
			res[1] = (int) ((gpsLocation.getLatitude() - center.getLatitude())
					* ARC_DISTANCE_PER_DEGREE * pixelsPerMeter * -1);
			res[0] = (int) ((gpsLocation.getLongitude() - center.getLongitude())
					* ARC_DISTANCE_PER_DEGREE
					* Math.cos(Math.toRadians((center.getLatitude() + gpsLocation
							.getLatitude()) / 2)) * pixelsPerMeter);
		}
	...
	private Location convertPixelToGPSLocation(double x, double y,
		...
		if (center != null) {
			...
			double longitude = center.getLongitude()
					+ x
					/ (ARC_DISTANCE_PER_DEGREE * Math.cos(Math
							.toRadians((center.getLatitude() + latitude) / 2)));
	...
	public synchronized void drawToCanvas(Canvas canvas, Location center, Rect boundingBox,
		...
		for (int i = 0; i < this.size() - 1; i++) {
			...
			int[] bpix = convertToPixelLocation(b, center,
					pixelsPerMeterOrMaxValue);
			canvas.drawLine(w + apix[0], h + apix[1], w + bpix[0], h + bpix[1],
					pLine);
		}
	...
	public String exportData(String prefix, String postfix) {
		...
		if (mExternalStorageAvailable && mExternalStorageWriteable) {
			...
			while (f.exists()) {
				index++;
				filePath = dir.toString() + "/" + prefix + "." + index + "."
						+ postfix;
				;
				f = new File(filePath);
			}
			...
			for (int i = 0; i < this.size(); i++) {
				...
				if (l != null) {
					builder.append("" + (l.getTime() - ms_init) + ","
